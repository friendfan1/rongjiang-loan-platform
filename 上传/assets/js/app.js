(function () {
  ...
  function getDataUrl() {
    const configured = window.SITE_CONFIG && window.SITE_CONFIG.dataUrl;
    if (configured) return configured;

    const rel = "data/products.json";
  ...
  function formatContactHtml(contact) {
    ...
    return text.replace(/(1[3-9]\d{9}|0\d{2,3}-?\d{7,8})/g, (num) => {
      const tel = num.replace(/-/g, "");
      return `<a href="tel:${tel}" class="contact-tel">${num}</a>`;
    });
  }
  ...
  function openDetail(id) {
    ...
    if (p.process) {
      bodyHtml += `<section class="detail-section"><h3 class="detail-section-title">申请流程</h3>${formatProcessText(p.process)}</section>`;
    }

    if (p.contact) {
      bodyHtml += `<section class="detail-section detail-contact"><h3 class="detail-section-title">联系咨询</h3><div class="contact-box"><span class="contact-icon" aria-hidden="true">☎</span><div class="contact-content"><span class="contact-label">联系人及电话</span><span class="contact-value">${formatContactHtml(p.contact)}</span></div></div></section>`;
    }
  ...
  async function init() {
    ...
    if (window.location.protocol === "file:") {
      els.resultSummary.textContent = "请通过 Web 服务器访问";
      els.productList.innerHTML =
        '<div class="empty-state">不能直接双击打开 HTML 文件。请用 Nginx 部署，或在本机运行：<br><code>python -m http.server 8765</code><br>然后访问 <code>http://localhost:8765</code></div>';
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("HTTP " + res.status + "，请求 " + dataUrl);
      }
      state.data = await res.json();
      if (!state.data.products || !state.data.products.length) {
      ...
      els.resultSummary.textContent = "数据加载失败";
      els.productList.innerHTML =
        ...
        "<p>请检查：</p>" +
        "<ul>" +
        "<li>服务器上是否存在 <code>data/products.json</code></li>" +
        "<li>本地可先运行 <code>python scripts/import_xlsx.py</code> 生成数据</li>" +
        "<li>Nginx 部署需上传完整目录（含 data、assets）</li>" +
        "</ul></div>";
    }
  }
